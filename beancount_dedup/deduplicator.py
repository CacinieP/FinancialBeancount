...
from .models import (
    Transaction, Platform, DedupStatus, DedupResult, DeduplicationReport
)
...
class DeduplicationEngine:
    ...
    TRANSFER_KEYWORDS = {
        ...
        "bank": ["支付宝充值", "财付通充值", "快捷支付退款", "支付宝提现入账", 
                "微信零钱充值", "微信零钱提现"],
    }
    
    def __init__(self, fingerprinter: Optional[TransactionFingerprinter] = None):
    ...
    def resolve_priority(self, tx1: Transaction, tx2: Transaction) -> Tuple[Transaction, Transaction]:
        """
        决定保留哪个交易
    ...
    def _check_l1_duplicate(self, tx: Transaction, fingerprints: Dict) -> Optional[Transaction]:
        """
        L1精确匹配检查
    ...
    def _check_l2_duplicate(self, tx: Transaction, fingerprints: Dict) -> Optional[Transaction]:
        """
        L2宽松匹配检查
        """
        # 检查主L2指纹
        ...
        if l2_hash in self.l2_index:
            for candidate in self.l2_index[l2_hash]:
                ...
                if tx.platform == candidate.platform:
                    continue
                if self.fingerprinter.check_l2_match(tx, candidate, 
                                                      fingerprints, 
    ...
    def _check_l3_candidates(self, tx: Transaction, fingerprints: Dict) -> List[Tuple[Transaction, Dict]]:
        """
        L3模糊匹配搜索
        ...
        # 遍历L2索引中的所有交易（使用L2作为初步筛选）
        for l2_hash, tx_list in self.l2_index.items():
            for candidate in tx_list:
                # 确保候选交易有完整的指纹
    ...
    def add_transaction(self, tx: Transaction) -> DedupResult:
        ...
        if result.status == DedupStatus.UNIQUE:
            ...
            if l1_match:
                ...
                if self.detect_transfer_pair(tx, l1_match):
                    ...
                    keeper, discarder = self.resolve_priority(tx, l1_match)
                    is_kept = (tx == keeper)
        ...
        if result.status == DedupStatus.UNIQUE:
            ...
            if l2_match:
                keeper, discarder = self.resolve_priority(tx, l2_match)
                is_kept = (tx == keeper)
                result.status = DedupStatus.DUPLICATE if not is_kept else DedupStatus.UNIQUE
        ...
        if result.status == DedupStatus.UNIQUE:
            ...
            if l3_candidates:
                # 找到最可能的匹配（时间差最小的）
                best_match, best_info = min(l3_candidates, 
                                            key=lambda x: x[1]["time_diff_seconds"])
    ...
    def get_unique_transactions(self) -> List[Transaction]:
        """获取去重后的唯一交易列表"""
        return [tx for tx in self.processed 
